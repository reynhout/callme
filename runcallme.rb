...
cb_proc = USE_RUBY_CALLBACK ? Proc.new { |x| print "(Ruby:#{x})" } : nil

puts "-- Passing callback address (#{cb_proc.inspect})"
...
if USE_RUBY_THREADS
  Thread.new { CallMe.watch_path("somefile", cb_proc, USE_C_THREADS) }
else
  CallMe.watch_path("somefile", cb_proc, USE_C_THREADS)
end
